...
do while(j<20)
    ...
    do i=1,N-1
        ...
        yl(1)=yzl*detx+yl(0) 
        !passo seguinte 
        yl(i+1)=2*yl(i)-yl(i-1)-2*detx*detx*g(i*detx-L,E,k1,k2)*yl(i) 
        !condições iniciais para direita
        yr(N)=yz
        yr(N-1)=yzl*detx+yr(N)     
        !sentido contrário
        yr(N-(i+1))=2*yr(N-(i))-yr(N-(i-1))-2*detx*detx*g(L-i*detx,E,k1,k2)*yr(N-(i)) 
    enddo
    !ponto de encontro
    xm=5700
    !derivada
...
function g(x,E,k1,k2)
    real*8, intent (in) :: x,E,k1,k2
    real*8 :: g
    g = (E-k1*x*x/2.0-k2*x*x*x*x/2.0) 
end function